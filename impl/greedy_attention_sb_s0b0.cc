...
struct ParserBuilder {
  ...
  explicit ParserBuilder(Model* model, const unordered_map<unsigned, vector<float>>& pretrained) :
      ...
      p_s02att(model->add_parameters({ATTENTION_HIDDEN_DIM, BILSTM_HIDDEN_DIM*2})),
      ...
      p_b02att(model->add_parameters({ATTENTION_HIDDEN_DIM, BILSTM_HIDDEN_DIM*2})),
...
Expression log_prob_parser(ComputationGraph* hg,
    ...
    Expression s02att = parameter(*hg, p_s02att);
    ...
    Expression b02att = parameter(*hg, p_b02att);
...
if(DEBUG)	std::cerr<<"possible action " << current_valid_actions.size()<<"\n";
      ...
      Expression s0 = (stack_buffer_split == 0 ? sent_start_expr : input[stack_buffer_split-1]);
      s_att.push_back(tanh(affine_transform({s_attbias, s_input2att, sent_start_expr, s_h2att, prev_h, s02att, s0})));
      s_input.push_back(sent_start_expr);
      for(unsigned i = 0; i < stack_buffer_split; i ++){
        s_att.push_back(tanh(affine_transform({s_attbias, s_input2att, input[i], s_h2att, prev_h, s02att, s0})));
      ...
      Expression b0 = (stack_buffer_split == sent.size() ? sent_end_expr : input[stack_buffer_split]);
      for(unsigned i = stack_buffer_split; i < sent.size(); i ++){
        b_att.push_back(tanh(affine_transform({b_attbias, b_input2att, input[i], b_h2att, prev_h, b02att, b0})));
        b_input.push_back(input[i]);
      }
      b_att.push_back(tanh(affine_transform({b_attbias, b_input2att, sent_end_expr, b_h2att, prev_h, b02att, b0})));
