...
if(DEBUG)	std::cerr<<"possible action " << current_valid_actions.size()<<"\n";
	...
	*/
      ...
      Expression s_att_pool = (stack_buffer_split == 0 ? sent_start_expr : input[stack_buffer_split-1]);
      Expression b_att_pool = (stack_buffer_split == sent.size() ? sent_end_expr : input[stack_buffer_split]);
      //
