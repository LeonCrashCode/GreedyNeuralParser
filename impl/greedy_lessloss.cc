...
void normalize_digital_lower(string& line){
  for(unsigned i = 0; i < line.size(); i ++){
    if(pretrained.size() == 0 && line[i] >= '0' && line[i] <= '9'){
      line[i] = '0';
    }
...
*/
	...
	unsigned size(){ return raws.size();}
...
int main(int argc, char** argv) {
  dynet::initialize(argc, argv, 1989121013);
...
//  for(int i = 0; i < labels.size(); i ++) ld.convert(labels[i]);
  ...
  if(conf.count("pretrained")){
    cerr << "Loading from " << conf["pretrained"].as<string>() << " with " << WORD_DIM << " dimensions ... ";
    ifstream in(conf["pretrained"].as<string>().c_str());
    string word;
    while(in>>word){
      vector<float> v(WORD_DIM);
      for(unsigned i = 0; i < WORD_DIM; i++) {in>>v[i];}
