...
struct ParserBuilder {
  ...
  explicit ParserBuilder(Model* model, const unordered_map<unsigned, vector<float>>& pretrained) :
      ...
      p_input2att(model->add_parameters({ATTENTION_HIDDEN_DIM, BILSTM_HIDDEN_DIM*2})),
      p_h2att(model->add_parameters({ATTENTION_HIDDEN_DIM, STATE_HIDDEN_DIM})),
      p_attbias(model->add_parameters({ATTENTION_HIDDEN_DIM})),
      p_att2attexp(model->add_parameters({ATTENTION_HIDDEN_DIM})),
      p_att2combo(model->add_parameters({STATE_HIDDEN_DIM, BILSTM_HIDDEN_DIM*2})),
...
Expression log_prob_parser(ComputationGraph* hg,
    ...
    Expression input2att = parameter(*hg, p_input2att);
    Expression h2att = parameter(*hg, p_h2att);
    Expression attbias = parameter(*hg, p_attbias);
    Expression att2attexp = parameter(*hg, p_att2attexp);
  
    Expression att2combo = parameter(*hg, p_att2combo);
...
if(DEBUG)	std::cerr<<"possible action " << current_valid_actions.size()<<"\n";
      //attention
      vector<Expression> att(sent.size());
      for(unsigned i = 0; i < sent.size(); i ++){
        att[i] = tanh(affine_transform({attbias, input2att, input[i], h2att, prev_h}));
      }
...
if(DEBUG)	std::cerr<<"attention ok\n";
      Expression h = state_lstm.add_input(concatenate({prev_action, att_pool}));
      prev_h = h;
if(DEBUG)	std::cerr<<"state lstm ok\n";
      Expression combo = affine_transform({combobias, h2combo, h, att2combo, att_pool});
