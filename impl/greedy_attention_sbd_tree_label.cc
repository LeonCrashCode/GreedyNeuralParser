...
struct ParserBuilder {
  ...
  //treeLSTM ///////////
  Parameter p_tree_ig_rel;
  Parameter p_tree_ig_head_h;
  Parameter p_tree_ig_head_c;
  Parameter p_tree_ig_ldep_h;
  Parameter p_tree_ig_ldep_c;
  Parameter p_tree_ig_rdep_h;
  Parameter p_tree_ig_rdep_c;
  Parameter p_tree_ig_bias;

  Parameter p_tree_hfg_rel;
  Parameter p_tree_hfg_head_h;
  Parameter p_tree_hfg_head_c;
  Parameter p_tree_hfg_ldep_h;
  Parameter p_tree_hfg_ldep_c;
  Parameter p_tree_hfg_rdep_h;
  Parameter p_tree_hfg_rdep_c;
  Parameter p_tree_hfg_bias;

  Parameter p_tree_dfg_rel;
  Parameter p_tree_ldfg_head_h;
  Parameter p_tree_ldfg_head_c;
  Parameter p_tree_ldfg_dep_h;
  Parameter p_tree_ldfg_dep_c;
  Parameter p_tree_ldfg_bias;
  Parameter p_tree_rdfg_head_h;
  Parameter p_tree_rdfg_head_c;
  Parameter p_tree_rdfg_dep_h;
  Parameter p_tree_rdfg_dep_c;
  Parameter p_tree_rdfg_bias;
 
  Parameter p_tree_ic_rel;
  Parameter p_tree_ic_head_h;
  Parameter p_tree_ic_ldep_h;
  Parameter p_tree_ic_rdep_h;
  Parameter p_tree_ic_bias;

  Parameter p_tree_og_rel;
  Parameter p_tree_og_head_h;
  Parameter p_tree_og_head_c;
  Parameter p_tree_og_ldep_h;
  Parameter p_tree_og_ldep_c;
  Parameter p_tree_og_rdep_h;
  Parameter p_tree_og_rdep_c;
  Parameter p_tree_og_bias;
  ...
  explicit ParserBuilder(Model* model, const unordered_map<unsigned, vector<float>>& pretrained) :
      state_lstm(LAYERS, STATE_INPUT_DIM ,STATE_HIDDEN_DIM, model),
      ...
      ////treeLSTM
      p_tree_ig_rel(model->add_parameters({BILSTM_HIDDEN_DIM*2, REL_DIM})),
      p_tree_ig_head_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ig_head_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ig_ldep_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ig_ldep_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ig_rdep_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ig_rdep_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ig_bias(model->add_parameters({BILSTM_HIDDEN_DIM*2})),
      p_tree_hfg_rel(model->add_parameters({BILSTM_HIDDEN_DIM*2, REL_DIM})),
      p_tree_hfg_head_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_hfg_head_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_hfg_ldep_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_hfg_ldep_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_hfg_rdep_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_hfg_rdep_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_hfg_bias(model->add_parameters({BILSTM_HIDDEN_DIM*2})),
      p_tree_dfg_rel(model->add_parameters({BILSTM_HIDDEN_DIM*2, REL_DIM})),
      p_tree_ldfg_head_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ldfg_head_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ldfg_dep_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ldfg_dep_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ldfg_bias(model->add_parameters({BILSTM_HIDDEN_DIM*2})),
      p_tree_rdfg_head_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_rdfg_head_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_rdfg_dep_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_rdfg_dep_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_rdfg_bias(model->add_parameters({BILSTM_HIDDEN_DIM*2})),
      p_tree_ic_rel(model->add_parameters({BILSTM_HIDDEN_DIM*2, REL_DIM})),
      p_tree_ic_head_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ic_ldep_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ic_rdep_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_ic_bias(model->add_parameters({BILSTM_HIDDEN_DIM*2})),
      p_tree_og_rel(model->add_parameters({BILSTM_HIDDEN_DIM*2, REL_DIM})),
      p_tree_og_head_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_og_head_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_og_ldep_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_og_ldep_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_og_rdep_h(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_og_rdep_c(model->add_parameters({BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_tree_og_bias(model->add_parameters({BILSTM_HIDDEN_DIM*2})),
      ///////////////////
...
vector<Expression> treeLSTM(ComputationGraph* hg,
			Expression head_h,
			Expression head_c,
			Expression dep_h,
			Expression dep_c,
			...
			bool left){
	//input gate
	Expression i_tree_ig_rel = parameter(*hg, p_tree_ig_rel);	
	Expression i_tree_ig_head_h = parameter(*hg, p_tree_ig_head_h);
	Expression i_tree_ig_head_c = parameter(*hg, p_tree_ig_head_c);
	Expression i_tree_ig_dep_h = (left ? parameter(*hg, p_tree_ig_ldep_h): parameter(*hg, p_tree_ig_rdep_h));
	Expression i_tree_ig_dep_c = (left ? parameter(*hg, p_tree_ig_ldep_c): parameter(*hg, p_tree_ig_rdep_c));
	Expression i_tree_ig_bias = parameter(*hg, p_tree_ig_bias);

	Expression igate = logistic(affine_transform({i_tree_ig_bias,
					i_tree_ig_rel, relation,
					i_tree_ig_head_h, head_h,
					i_tree_ig_head_c, head_c,
					i_tree_ig_dep_h, dep_h,
					i_tree_ig_dep_c, dep_c}));

	//head forget gate
	Expression i_tree_hfg_rel = parameter(*hg, p_tree_hfg_rel);
	Expression i_tree_hfg_head_h = parameter(*hg, p_tree_hfg_head_h);
        Expression i_tree_hfg_head_c = parameter(*hg, p_tree_hfg_head_c);
        Expression i_tree_hfg_dep_h = (left ? parameter(*hg, p_tree_hfg_ldep_h): parameter(*hg, p_tree_hfg_rdep_h));
        Expression i_tree_hfg_dep_c = (left ? parameter(*hg, p_tree_hfg_ldep_c): parameter(*hg, p_tree_hfg_rdep_c));
        Expression i_tree_hfg_bias = parameter(*hg, p_tree_hfg_bias); 

	Expression hfgate = logistic(affine_transform({i_tree_hfg_bias,
					i_tree_hfg_rel, relation,
                                        i_tree_hfg_head_h, head_h,
                                        i_tree_hfg_head_c, head_c,
                                        i_tree_hfg_dep_h, dep_h,
                                        i_tree_hfg_dep_c, dep_c}));

	//dep forget gate
	Expression i_tree_dfg_rel = parameter(*hg, p_tree_dfg_rel);
	Expression i_tree_dfg_head_h = (left ? parameter(*hg, p_tree_ldfg_head_h): parameter(*hg, p_tree_rdfg_head_h));
        Expression i_tree_dfg_head_c = (left ? parameter(*hg, p_tree_ldfg_head_c): parameter(*hg, p_tree_rdfg_head_c));
        Expression i_tree_dfg_dep_h = (left ? parameter(*hg, p_tree_ldfg_dep_h): parameter(*hg, p_tree_rdfg_dep_h));
        Expression i_tree_dfg_dep_c = (left ? parameter(*hg, p_tree_ldfg_dep_c): parameter(*hg, p_tree_rdfg_dep_c));
        Expression i_tree_dfg_bias = (left ? parameter(*hg, p_tree_ldfg_bias): parameter(*hg, p_tree_rdfg_bias));

        Expression dfgate = logistic(affine_transform({i_tree_dfg_bias,
					i_tree_dfg_rel, relation,
                                        i_tree_dfg_head_h, head_h,
                                        i_tree_dfg_head_c, head_c,
                                        i_tree_dfg_dep_h, dep_h,
                                        i_tree_dfg_dep_c, dep_c}));
	//input cell
	Expression i_tree_ic_rel = parameter(*hg, p_tree_ic_rel);
	Expression i_tree_ic_head_h = parameter(*hg, p_tree_ic_head_h);
        Expression i_tree_ic_dep_h = (left ? parameter(*hg, p_tree_ic_ldep_h): parameter(*hg, p_tree_ic_rdep_h));
        Expression i_tree_ic_bias = parameter(*hg, p_tree_ic_bias);

	Expression icell = tanh(affine_transform({i_tree_ic_bias,
                                        i_tree_ic_rel, relation,
					i_tree_ic_head_h, head_h,
                                        i_tree_ic_dep_h, dep_h}));

	//new cells
	vector<Expression> cells;
	cells.push_back(cwise_multiply(igate, icell));
	cells.push_back(cwise_multiply(hfgate, head_c));
	cells.push_back(cwise_multiply(dfgate, dep_c));
	
	Expression newcell = sum(cells);

	//output gate
	Expression i_tree_og_rel = parameter(*hg, p_tree_og_rel);
	Expression i_tree_og_head_h = parameter(*hg, p_tree_og_head_h);
        Expression i_tree_og_head_c = parameter(*hg, p_tree_og_head_c);
        Expression i_tree_og_dep_h = (left ? parameter(*hg, p_tree_ig_ldep_h): parameter(*hg, p_tree_ig_rdep_h));
        Expression i_tree_og_dep_c = (left ? parameter(*hg, p_tree_og_ldep_h): parameter(*hg, p_tree_og_rdep_c));
        Expression i_tree_og_bias = parameter(*hg, p_tree_og_bias);

        Expression ogate = logistic(affine_transform({i_tree_og_bias,
                                        i_tree_og_rel, relation,
					i_tree_og_head_h, head_h,
                                        i_tree_og_head_c, head_c,
                                        i_tree_og_dep_h, dep_h,
                                        i_tree_og_dep_c, dep_c}));

	Expression h = cwise_multiply(ogate, tanh(newcell));
...
if(DEBUG)	std::cerr<<"\nlookup table ok\n";
    ...
    vector<Expression> l2r_c(sent.size());
    vector<Expression> r2l_c(sent.size());
    ...
    for (unsigned i = 0; i < sent.size(); ++i) {
      ...
      l2r_c[i] = l2rbuilder.c.back()[0];
      r2l[sent.size() - 1 - i] = r2lbuilder.add_input(input_expr[sent.size()-1-i]);
      r2l_c[sent.size() - 1 - i] = r2lbuilder.c.back()[0];
    }
    ...
    vector<Expression> input_c(sent.size());
    vector<unsigned> indexes(sent.size());
    vector<bool> heads(sent.size());
   
    for (unsigned i = 0; i < sent.size(); ++i) {
      input[i] = concatenate({l2r[i],r2l[i]});
      input_c[i] = concatenate({l2r_c[i], r2l_c[i]});
      indexes[i] = i;
      heads[i] = true;
    }
...
if(DEBUG)	std::cerr<<"/////////////////action index " << action_count<<" sentence size:"<< sent.size()<<"\n";
...
if(DEBUG)	std::cerr<<"\n non terminal";
      for(unsigned i = sent.size(); i < heads.size(); i ++){
        if(!heads[i]) continue;
if(DEBUG)	std::cerr<<i-sent.size()<<" ";
	s_att.push_back(tanh(affine_transform({s_attbias, s_input2att, partial[i-sent.size()], s_h2att, prev_h})));
	s_input.push_back(partial[i-sent.size()]);
      }
if(DEBUG)	std::cerr<<"\n";
...
if(DEBUG)	std::cerr<<"gold action " << setOfActions[action] << "\n";
...
if(DEBUG)	std::cerr<<"action lookup ok\n";
	...
	assert(heads[indexes[headi]]);
	assert(heads[indexes[depi]]);

	Expression head_h = (indexes[headi] < sent.size() ? input[indexes[headi]] : partial[indexes[headi]-sent.size()]);
	Expression head_c = (indexes[headi] < sent.size() ? input_c[indexes[headi]] : partial_c[indexes[headi]-sent.size()]);
	
	Expression dep_h = (indexes[depi] < sent.size() ? input[indexes[depi]] : partial[indexes[depi]-sent.size()]);
        Expression dep_c = (indexes[depi] < sent.size() ? input_c[indexes[depi]] : partial_c[indexes[depi]-sent.size()]);

	vector<Expression> combination = treeLSTM(hg, head_h, head_c, dep_h, dep_c, relation, left);

if(DEBUG)	std::cerr<<"indexes for depi: "<< indexes[depi] <<", indexes for headi: "<<indexes[headi]<<"\n";
	heads[indexes[depi]] = false;
	heads[indexes[headi]] = false;
	indexes[headi] = heads.size();
	heads.push_back(true);

	partial.push_back(combination[0]);
	partial_c.push_back(combination[1]);
