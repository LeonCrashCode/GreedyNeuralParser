...
void InitCommandLine(int argc, char** argv, po::variables_map* conf) {
  ...
  opts.add_options()
	...
	("wins", po::value<unsigned>()->default_value(1),"widow size")
...
struct ParserBuilder {
  ...
  LookupParameter p_swins;
  LookupParameter p_bwins;
  ...
  explicit ParserBuilder(Model* model, const unordered_map<unsigned, vector<float>>& pretrained) :
      ...
      p_swins(model->add_lookup_parameters(512, {BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      p_bwins(model->add_lookup_parameters(512, {BILSTM_HIDDEN_DIM*2, BILSTM_HIDDEN_DIM*2})),
      ...
      p_Cbias(model->add_parameters({BILSTM_HIDDEN_DIM*2})),
      p_C2combo(model->add_parameters({STATE_HIDDEN_DIM, BILSTM_HIDDEN_DIM*2})),
...
Expression log_prob_parser(ComputationGraph* hg,
    ...
    Expression C2combo = parameter(*hg, p_C2combo);
...
if(DEBUG)	std::cerr<<"possible action " << current_valid_actions.size()<<"\n";
	...
	*/
      vector<Expression> args = {Cbias};
      for (unsigned i = 0; i < WINS; ++i){
	Expression swin = lookup(*hg, p_swins, i);
	if(train) swin = dropout(swin, pdrop);
	args.push_back(swin);
      	if(stack_buffer_split == i) args.push_back(sent_start_expr);
	else if(stack_buffer_split < i) args.push_back(zeroes(*hg, {BILSTM_HIDDEN_DIM*2}));
	else args.push_back(input[stack_buffer_split - i - 1]);
      }

      for (unsigned i = 0; i < WINS; ++i){
	Expression bwin = lookup(*hg, p_bwins, i);
        if(train) bwin = dropout(bwin, pdrop);
        args.push_back(bwin);
        if(stack_buffer_split + i == sent.size()) args.push_back(sent_start_expr);
        else if(stack_buffer_split + i > sent.size()) args.push_back(zeroes(*hg, {BILSTM_HIDDEN_DIM*2}));
        else args.push_back(input[stack_buffer_split + i]);
      }

      Expression C = rectify(affine_transform(args));
      
      //
if(DEBUG)	std::cerr<<"attention ok\n";
      Expression h = state_lstm.add_input(concatenate({prev_action, C}));
if(DEBUG)	std::cerr<<"state lstm ok\n";
      Expression combo = affine_transform({combobias, h2combo, prev_h, C2combo, C});
      prev_h = h;
